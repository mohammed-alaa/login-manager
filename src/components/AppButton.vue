<template>
    <transition name="fade" mode="out-in">
        <button :key="0"
            :class="`btn btn-${size} btn-${theme} ${isActive ? 'active' : ''} ${noPadding ? 'nopadding' : ''} ${noBorderRadius ? 'noborderradius' : ''} ${border ? 'border' : ''}`"
            :type="type" @click.prevent="$emit('click', $event)" :disabled="loading" :aria-disabled="loading"
            :tabindex="`${loading ? '-1' : '0'}`">
            <AppIcon v-if="loading" icon="spinner-third fa-spin" class="ms-2" :key="1" />
