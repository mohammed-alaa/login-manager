...
export default createStore({
  ...
  actions: {
    dispatchEvent({ commit }, { eventName, params = null, response = true }) {
      if (response) {
        return new Promise((resolve) => {
          commit("isLoading", true)
          window.ipcRenderer.invoke(eventName, params).then((eventResult) => {
    ...
    retrieveLogins({ commit, dispatch }) {
      dispatch("dispatchEvent", { eventName: "read-logins" }).then((logins) => {
    ...
    setViewMode({ dispatch, getters }) {
      ...
      dispatch("setMode", "v")
      isOldModeCreate &&
        dispatch("activeLoginChange", getters.getLoginList?.[0]?.id || -1)
    },
    createNewLogin({ dispatch, getters }, newLogin) {
      ...
      dispatch("dispatchEvent", {
        ...
        dispatch(
          "activeLoginChange",
          getters.getLoginList?.[getters.getLoginList.length - 1]?.id || "-1"
        )
      })
    },
